...
use crate::{
    ...
    server::{
        FNP, Peer,
        peerstore::{Connection, PeerStore},
    },
};
...
impl ServerBackend {
    ...
    pub async fn run(
        ...
        let listener_task = smol::spawn(async move { server_clone.listen(listener_sender).await });

        let server_clone2 = self.clone();
        let sendout_task = smol::spawn(async move { server_clone2.sendout(&msg_receiver).await });
            ...
            match msg {
                ...
                _ => {
                    let dest_addr = msg
                        .dest()
                        .expect(
                            "Protocolo gerado errado, mensages diretas devem ter um destinatário.",
                        )
                        .address();
                    if let Some(info) = self.peer_store().get_by_listener(&dest_addr).await {
                        ...
                        crate::tui::err(&format!(
                            "* Peer {} não encontrado na sua rede.",
                            dest_addr
                        ));
                    }
                }
